...
RC ExecuteStage::do_select(const char *db, const Selects &selects, SessionEvent *session_event, TupleSet &ret_tuple_set, bool is_sub_select)
{
  ...
  for (size_t i = 0; i < selects.condition_num; i++)
  {
    ...
    for (size_t i = 0; i < sub_select->condition_num; i++)
    {
      ...
      if (condition.right_is_attr == 1 && condition.right_attr.relation_name != nullptr && strcmp(condition.right_attr.relation_name, selects.relations[0]) == 0) {
        // 加入子查询
      ...
      if (condition.left_is_attr == 1 && condition.left_attr.relation_name != nullptr && strcmp(condition.left_attr.relation_name, selects.relations[0]) == 0) {
        // 加入子查询
        sub_select->relations[sub_select->relation_num++] = selects.relations[0];
        // 加上group by
        sub_select->group_num = 0;
